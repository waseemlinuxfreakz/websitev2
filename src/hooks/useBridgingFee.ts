...
function getTxFee(txReceipt: TransactionReceipt | undefined): number {
  if (txReceipt) {
    const _gasUsed: number = parseInt(txReceipt.gasUsed.toString());
    const _effectiveGasPrice: number = parseInt(
...
export default function useBridgingFee(
  ...
  const [fromStatus, setFromStatus] = useState<TTxStatus | undefined>(
    "pending",
  );
